       ...
       for ( ; cps_api_object_it_valid(&nhit) ;
           ...
           for ( ; cps_api_object_it_valid(&node) ;
               ...
               case cps_api_if_ROUTE_A_NEXT_HOP_ADDR:
                   ...
                   EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR(CPS): ",
...
int fib_create_dr_tree (t_fib_vrf_info *p_vrf_info)
{
    ...
    if (!p_vrf_info)
    {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-DR", "%s (): Invalid input param. p_vrf_info: %p\r\n",
                   __FUNCTION__, p_vrf_info);

        return (STD_ERR_MK(e_std_err_ROUTE, e_std_err_code_FAIL, 0));
    }

    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "Vrf_id: %d, af_index: %s\r\n",
                p_vrf_info->vrf_id,
               STD_IP_AFINDEX_TO_STR (p_vrf_info->af_index));

    if (p_vrf_info->dr_tree != NULL)
    {
        EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "DR tree already created. "
                   "vrf_id: %d, af_index: %d\r\n",
    ...
    if (p_vrf_info->dr_tree == NULL)
    {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-DR",
...
int fib_proc_dr_download (cps_api_object_t object)
{
    db_route_t   *p_rt_entry;
    db_route_t    rt_entry;
    ...
    cps_obj_to_route(object,&rt_entry);
    p_rt_entry = &rt_entry;
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT", "type: %d vrf_id %d, af-index %d"
            " route count %d, nh_count %d distance %d\r\n", p_rt_entry->msg_type, af_index,
            vrf_id, 1, p_rt_entry->hop_count, p_rt_entry->distance);

    if (!(FIB_IS_VRF_ID_VALID (vrf_id))) {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-DR", "%s (): Invalid vrf_id. vrf_id: %d\r\n",
                   __FUNCTION__, vrf_id);
        return STD_ERR_OK;
    }
    ...
    /*
     * The following check is to prevent configuring self ip address with /32 prefix in FIB/NPU
     * These routes comes with protocol as kernel.
     */
    if (p_rt_entry->protocol == RT_KERNEL && p_rt_entry->hop_count == 1
        && STD_IP_IS_ADDR_ZERO(&p_rt_entry->nh_list[0].nh_addr)) {
        if (af_index == HAL_RT_V4_AFINDEX && p_rt_entry->prefix_masklen == HAL_RT_V4_PREFIX_LEN) {
            EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "Skipping local addr %s on if_indx %d",
                                 FIB_IP_ADDR_TO_STR(&p_rt_entry->prefix), nh_if_index);
            return STD_ERR_OK;
        } else if (p_rt_entry->prefix_masklen == HAL_RT_V6_PREFIX_LEN) {
            EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "Skipping local addr %s on if_indx %d",
                                 FIB_IP_ADDR_TO_STR(&p_rt_entry->prefix), nh_if_index);
            return STD_ERR_OK;
        }
    ...
    switch (p_rt_entry->msg_type) {
        case ROUTE_ADD:
            ...
            fib_proc_dr_add_msg (af_index, p_rt_entry, &nh_info_size);
            rt_change = true;
            break;
        ...
        case ROUTE_DEL:
            ...
            break;
        default:
            EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-DR", "%s (): Invalid case. \r\n", __FUNCTION__);
            break;
    }
    if(rt_change) {
...
int fib_proc_dr_add_msg (uint8_t af_index, void *p_rtm_fib_cmd, int *p_nh_info_size)
{
    ...
    if (!p_rtm_fib_cmd) {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-DR", "%s (): Invalid input param. p_rtm_fib_cmd: %p\r\n",
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 1, "HAL-RT-DR(RT-START)",
    ...
    if (p_dr == NULL)
    {
        EV_LOG_TRACE(ev_log_t_ROUTE, 2, "HAL-RT-DR",
     ...
     * This shall impose limitation on native linux commands like
...
int fib_proc_dr_del_msg (uint8_t af_index, void *p_rtm_fib_cmd)
{
    ...
    if (p_dr == NULL)
    {
        EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR",
            ...
            if (p_dr->prefix_len == FIB_AFINDEX_TO_PREFIX_LEN(p_dr->key.prefix.af_index))
            {
                ...
                if (p_nh == NULL)
                {
                                        ...
                                        &(p_dr->key.prefix),sizeof(t_fib_ip_addr)))))
                    {
                        ...
                        {
                            EV_LOG_TRACE(ev_log_t_ROUTE, 3,"HAL-RT-DR", "Host with same prefix"
...
int fib_proc_dr_nh_add (t_fib_dr *p_dr, void *p_rtm_fib_cmd, int *p_nh_info_size)
{
    ...
    if ((!p_dr) || (!p_rtm_fib_cmd) || (!p_nh_info_size)) {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-DR", "%s (): Invalid input param. p_dr: %p,"
    ...
    af_index = p_dr->key.prefix.af_index;
    nh_count = ((db_route_t  *)p_rtm_fib_cmd)->hop_count;
    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "nh_count=%d vrf_id: %d, prefix: %s, prefix_len: %d\r\n",
...
{
    db_route_t  *p_rtm_v4_fib_cmd = NULL;
    db_route_t  *p_rtm_v6_fib_cmd = NULL;

    if ((!p_rtm_fib_cmd) || (!p_fib_dr_msg_info)) {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-DR", "%s (): Invalid input param. "
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "af_index: %d\r\n",  af_index);

    memset (p_fib_dr_msg_info, 0, sizeof (t_fib_dr_msg_info));

    if (STD_IP_IS_AFINDEX_V4 (af_index)) {
        p_rtm_v4_fib_cmd = (db_route_t *)p_rtm_fib_cmd;
        ...
        p_rtm_v6_fib_cmd = (db_route_t *)p_rtm_fib_cmd;
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "vrf_id: %d, prefix: %s, "
...
{
    db_route_t     *p_rtm_v4NHKey = NULL;
    db_route_t     *p_rtm_v6NHKey = NULL;
    ...
    if (STD_IP_IS_AFINDEX_V4 (af_index)) {
        p_rtm_v4NHKey = (db_route_t *) p_rtm_nh_key;
    ...
    {
        p_rtm_v6NHKey = (db_route_t *)p_rtm_nh_key;
...
t_fib_dr *fib_get_dr (uint32_t vrf_id, t_fib_ip_addr *p_prefix, uint8_t prefix_len)
{
    ...
    if (p_dr != NULL)
    {
        EV_LOG_TRACE(ev_log_t_ROUTE, 2, "HAL-RT-DR", "vrf_id: %d, prefix: %s/%d, p_dr: %p\r\n",
...
t_fib_dr *fib_get_first_dr (uint32_t vrf_id, uint8_t af_index)
{
    ...
    if (p_dr != NULL)
    {
        EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "vrf_id: %d, prefix: %s/%d\r\n",
                   vrf_id, FIB_IP_ADDR_TO_STR (&p_dr->key.prefix),
                   p_dr->prefix_len);
    }
...
int fib_dr_walker_main (void)
{
    ...
    for (vrf_id = FIB_MIN_VRF; vrf_id < FIB_MAX_VRF; vrf_id++) {
        for (af_index = FIB_MIN_AFINDEX; af_index < FIB_MAX_AFINDEX; af_index++) {
            ...
            if (p_vrf_info == NULL)
            {
                EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-DR", "Vrf info NULL. "
                          "vrf_id: %d, af_index: %d\r\n",
                           vrf_id, af_index);
                continue;
            }
    ...
    for ( ; ;)
    {
        ...
        EV_LOG_TRACE(ev_log_t_ROUTE, 2, "HAL-RT-DR", "Total DR processed %d",  tot_dr_processed);
...
int fib_dr_walker_call_back (std_radical_head_t *p_rt_head, va_list ap)
{
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 1, "HAL-RT-DR",
               "DR: vrf_id: %d, prefix: %s, prefix_len: %d, "
               "status_flag: 0x%x\r\n", p_dr->vrf_id,
               FIB_IP_ADDR_TO_STR (&p_dr->key.prefix), p_dr->prefix_len,
               p_dr->status_flag);
...
int fib_resume_dr_walker_thread (uint8_t af_index)
{
    ...
    if((retval = pthread_cond_signal( &fib_dr_cond )) != 0) {
        EV_LOG_TRACE(ev_log_t_ROUTE, 1, "HAL-RT-DR", "pthread cond signal failed %d", retval);
    }
    pthread_mutex_unlock( &fib_dr_mutex );
    return STD_ERR_OK;
}
