...
int fib_create_nh_tree (t_fib_vrf_info *p_vrf_info)
{
    ...
    if (!p_vrf_info)
    {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-NH", "Invalid input param. p_vrf_info: %p\r\n", p_vrf_info);

        return (STD_ERR_MK(e_std_err_ROUTE, e_std_err_code_FAIL, 0));
    }

    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH", "Vrf_id: %d, af_index: %s\r\n",
                 p_vrf_info->vrf_id, STD_IP_AFINDEX_TO_STR (p_vrf_info->af_index));

    if (p_vrf_info->nh_tree != NULL)
    {
        EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH",
    ...
    if (p_vrf_info->nh_tree == NULL)
    {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-NH",
                      ...
                      uint32_t if_index, t_fib_nh_owner_type owner_type, uint32_t owner_value)
{
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 1, "HAL-RT-NH",
                     ...
                     uint32_t owner_value)
{
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH",
                 ...
                 p_nh->key.if_index, owner_type, owner_value);
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 2, "HAL-RT-NH",
...
t_fib_nh *fib_add_nh (uint32_t vrf_id, t_fib_ip_addr *p_ip_addr, uint32_t if_index)
{
    ...
    if (p_nh == NULL)
    {
        EV_LOG_ERR(ev_log_t_ROUTE, 3, "HAL-RT-NH", "%s (): Memory alloc failed\r\n", __FUNCTION__);
...
int fib_del_nh_tunnel_fh (t_fib_nh *p_nh, t_fib_nh *p_fh)
{
    ...
    if (p_tunnel_fh == NULL)
    {
        EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH", "Tunnel FH is NULL.\r\n");
...
int fib_resolve_connected_tunnel_nh (uint32_t vrf_id, t_fib_nh *p_fh)
{
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH", "FH: vrf_id: %d, ip_addr: %s,"
               " if_index: 0x%x \r\n",
...
int fib_nh_walker_main (void)
{
    ...
    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH", "af_index: %d\r\n", af_index);
    ...
    for (vrf_id = FIB_MIN_VRF; vrf_id < FIB_MAX_VRF; vrf_id++) {
        for (af_index = FIB_MIN_AFINDEX; af_index < FIB_MAX_AFINDEX; af_index++) {
            ...
            if (p_vrf_info == NULL) {
                EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH", "Vrf info NULL. "
                             "vrf_id: %d, af_index: %d\r\n", vrf_id, af_index);
                continue;
            }
    ...
    for ( ; ;)
    {
        ...
        for (vrf_id = FIB_MIN_VRF; vrf_id < FIB_MAX_VRF; vrf_id++) {
            for (af_index = FIB_MIN_AFINDEX; af_index < FIB_MAX_AFINDEX; af_index++) {
                ...
                if ((p_vrf_info->dr_clear_on == true) ||
                    ...
                    EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH", "DR clear or HA in progress."
        ...
        EV_LOG_TRACE(ev_log_t_ROUTE, 2, "HAL-RT-NH", "Total NH processed %d",  tot_nh_processed);
        ...
        if (FIB_IS_NH_FH (p_nh))
        {
            ...
            if (FIB_IS_NH_WRITTEN (p_nh))
            {
                ...
                if (hal_err == DN_HAL_ROUTE_E_NONE)
                {
                    ...
                    {
                        EV_LOG_TRACE(ev_log_t_ROUTE, 3, "HAL-RT-NH", "Full length route is available for resolution "
                                "Vrf_id: %d, prefix %s\r\n",  p_nh->vrf_id,
                                FIB_IP_ADDR_TO_STR (&p_nh->key.ip_addr));
...
int fib_resume_nh_walker_thread (uint8_t af_index)
{
    ...
    if((retval = pthread_cond_signal( &fib_nh_cond)) != 0) {
        EV_LOG_TRACE(ev_log_t_ROUTE, 1, "HAL-RT-NH", "pthread cond signal failed %d", retval);
    }
    pthread_mutex_unlock( &fib_nh_mutex );
    return STD_ERR_OK;
}
