...
void fib_help (void)
{
    ...
    printf ("  fib_dump_all_vrf_peer_routing_config ()\r\n");
...
void fib_dump_config (void)
{
    ...
    printf ("  hw_ecmp_max_paths                    :  %d\r\n",
            (hal_rt_access_fib_config())->hw_ecmp_max_paths);
...
void fib_dump_all_vrf_peer_routing_config(void)
{
    ...
    for (vrf_id = FIB_MIN_VRF; vrf_id < FIB_MAX_VRF; vrf_id++) {
        ...
        for (itr = 0; itr < HAL_RT_MAX_PEER_ENTRY ; itr++) {
            printf("\r\n Vrf:%d peer-mac:%s status:%d \r\n",
                   vrf_id, hal_rt_mac_to_str (&p_vrf->peer_routing_config[itr].peer_mac_addr,
                                              p_buf, HAL_RT_MAX_BUFSZ),
                   p_vrf->peer_routing_config[itr].status);
        }
    }
}
